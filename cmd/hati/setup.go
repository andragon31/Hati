...
var setupCmd = &cobra.Command{
	...
	Long: `Setup Hati integration for various AI coding agents.
...
Examples:
  ...
  hati setup cursor`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		fmt.Println("\nRestart your AI agent to start using Hati!")
	},
}
...
func (i *ClaudeCodeInstaller) Install() error {
	...
	dest := filepath.Join(dir, "hati.json")
...
func (i *CursorInstaller) Install() error {
	...
	mcpServers["hati"] = map[string]interface{}{"type": "command", "command": utils.ResolveBinaryPath(), "args": []string{"mcp"}, "env": make(map[string]string)}
...
func (i *WindsurfInstaller) Install() error {
	...
	mcpServers["hati"] = map[string]interface{}{"command": utils.ResolveBinaryPath(), "args": []string{"mcp"}}
	finalJSON, _ := json.MarshalIndent(config, "", "  ")
	return os.WriteFile(configPath, finalJSON, 0644)
}
