...
Write-Host "  Hati Installer $VERSION" -ForegroundColor Cyan
Write-Host "======================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "  >> AI Execution & Action Layer <<" -ForegroundColor Gray
Write-Host "  --------------------------------" -ForegroundColor Gray
Write-Host ""

if (Test-Path $EXE_PATH) {
    Write-Host "Hati already installed. Updating..." -ForegroundColor Yellow
}

Write-Host "[1/3] Compiling Hati (requires Go)..." -ForegroundColor DarkCyan
try {
    if (-not (Test-Path $INSTALL_DIR)) {
        New-Item -ItemType Directory -Force -Path $INSTALL_DIR | Out-Null
    }
    go build -o $EXE_PATH ./cmd/hati
} catch {
...
Write-Host "  hati init            # Initialize Hati in project"
Write-Host "  hati setup cursor    # Setup for Cursor"
Write-Host "  hati tui             # Open Dashboard"
Write-Host ""

if ($pathAdded) {
    Write-Host "NOTE: If 'hati' is not found, open a new PowerShell window." -ForegroundColor Yellow
}
